...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        ...
        self.encoder = nn.LSTM(input_size=embed_size, hidden_size=hidden_size, bidirectional=True) #(Bidirectional LSTM with bias)
        self.decoder = nn.LSTM(input_size=embed_size+hidden_size, hidden_size=hidden_size) #(LSTM Cell with bias)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        ### Except replace
        # "self.model_embeddings.source"
        # with "self.model_embeddings_source"
