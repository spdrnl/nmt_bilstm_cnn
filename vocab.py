...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        def get_char_indices(w):
            return [self.start_of_word] + [self.char2id.get(c, self.char_unk) for c in w] + [self.end_of_word]

        return [[get_char_indices(w) for w in s] for s in sents]
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_var = torch.Tensor(padded_sents).long().transpose(0, 1)
        return sents_var
        ### END YOUR CODE
